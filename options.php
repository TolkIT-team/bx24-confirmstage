...
.stage-url-block input[type="text"] {
    width: 100%;
...
<form method="post" id="configForm">
    ...
    <div id="configContainer">
        <?php foreach ($config as $index => $group): ?>
            <div class="adm-detail-block" data-index="<?= $index ?>" data-category=<?= $group['ID']; ?>>
                    ...
                    <table class="adm-detail-content-table edit-table">
                        ...
                        <tr>
                            ...
                            <td width="60%" class="adm-detail-content-cell-r">
                                <input type="text" name="CONFIG[<?= $index ?>][PATH]" size="60" class="adm-input"
                                       placeholder="example_page/example_section"
                                       value="<?= $group['PATH'] ?>">
                            </td>
                        </tr>
                        <tr>
                        ...
                        <tr>
                            ...
                            <td width="60%" class="adm-detail-content-cell-r">
                                <select disabled name="CONFIG[<?= $index ?>][PIPELINE_ID]" class="pipeline-select"
                                        data-entity="<?= $group['PIPELINE_ID'] ?? '' ?>">
                        ...
                        <?php if (!empty($group['STAGES'])): ?>
                            <?php foreach ($group['STAGES'] as $stageCode => $data): ?>
                                <tr class="stage-row">
                                    ...
                                    <td width="60%" class="adm-detail-content-cell-r">
                                        <textarea name="CONFIG[<?= $index ?>][STAGES][<?= htmlspecialcharsbx($stageCode) ?>][MESSAGE]" rows="2" class="adm-input" placeholder="Текст подтверждения"><?= htmlspecialcharsbx($data['MESSAGE'] ?? '') ?></textarea>

                                        <div class="stage-header">
                                            <input type="text" name="CONFIG[<?= $index ?>][STAGES][<?= htmlspecialcharsbx($stageCode) ?>][DIALOG_TITLE]" value="<?= htmlspecialcharsbx($data['DIALOG_TITLE'] ?? '') ?>" placeholder="Заголовок диалогового окна">
                                            <label>
                                                <input type="checkbox" name="CONFIG[<?= $index ?>][STAGES][<?= htmlspecialcharsbx($stageCode) ?>][IS_DISABLE_CHANGE]" <?= !empty($data['IS_DISABLE_CHANGE']) ? 'checked' : '' ?>>
                                                Остановить изменение стадии
                                            </label>
                                        </div>

                                        <div class="stage-url-block">
                                            <input type="text" name="CONFIG[<?= $index ?>][STAGES][<?= htmlspecialcharsbx($stageCode) ?>][URL]" value="<?= htmlspecialcharsbx($data['URL'] ?? '') ?>" placeholder="URL запроса">
                                            <input type="text" name="CONFIG[<?= $index ?>][STAGES][<?= htmlspecialcharsbx($stageCode) ?>][SUCCESS_MESSAGE]" value="<?= htmlspecialcharsbx($data['SUCCESS_MESSAGE'] ?? '') ?>" placeholder="Сообщение при успешном выполнении">
                                            <input type="text" name="CONFIG[<?= $index ?>][STAGES][<?= htmlspecialcharsbx($stageCode) ?>][ERROR_MESSAGE]" value="<?= htmlspecialcharsbx($data['ERROR_MESSAGE'] ?? '') ?>" placeholder="Сообщение при ошибке">
                                        </div>
...
BX.ready(function () {
        ...
        <div class="adm-detail-content-item-block">
            <table class="adm-detail-content-table edit-table">
                ...
                <tr>
                    <td width="40%" class="adm-detail-content-cell-l"><b>CRM сущность:</b></td>
                    <td width="60%" class="adm-detail-content-cell-r">
                        <select name="CONFIG[${index}][ENTITY]" class="entity-select">${options}</select>
                    </td>
                </tr>
                <tr>
                    <td width="40%" class="adm-detail-content-cell-l"><b>Воронка:</b></td>
    ...
    function loadPipelines(group, isFirstLoad = false) {
        ...
        BX.ajax.post('/bitrix/admin/tolkit_confirmstage_ajax.php', {
            ...
            try {
                ...
                pipelines.forEach(p => {
                    const selected = p.ID == pipelineSelect.dataset.entity ? 'selected' : '';
    ...
    function renderStages(stages, index, table) {
        ...
        stages.forEach(stage => {
            ...
            tr.innerHTML = `
                ...
                <td width="60%" class="adm-detail-content-cell-r">
                    <textarea name="CONFIG[${index}][STAGES][${BX.util.htmlspecialchars(stage.CODE)}][MESSAGE]" rows="2" class="adm-input" placeholder="Текст подтверждения"></textarea>
                    <div class="stage-header">
                        <input type="text" name="CONFIG[${index}][STAGES][${BX.util.htmlspecialchars(stage.CODE)}][DIALOG_TITLE]" placeholder="Заголовок диалогового окна">
                        <label>
                            <input type="checkbox" name="CONFIG[${index}][STAGES][${BX.util.htmlspecialchars(stage.CODE)}][IS_DISABLE_CHANGE]"> Остановить изменение стадии
                        </label>
                    </div>
                    <div class="stage-url-block">
                        <input type="text" name="CONFIG[${index}][STAGES][${BX.util.htmlspecialchars(stage.CODE)}][URL]" placeholder="URL запроса">
                        <input type="text" name="CONFIG[${index}][STAGES][${BX.util.htmlspecialchars(stage.CODE)}][SUCCESS_MESSAGE]" placeholder="Сообщение при успешном выполнении">
                        <input type="text" name="CONFIG[${index}][STAGES][${BX.util.htmlspecialchars(stage.CODE)}][ERROR_MESSAGE]" placeholder="Сообщение при ошибке">
                    </div>
